...
function getDirectAnswerFromTrainingData($question) {
    try {
        ...
        if (strpos($question_lower, '科系') !== false || strpos($question_lower, '科') !== false || strpos($question_lower, '系') !== false) {
            $sql = "SELECT question, answer, source_type, source_label, department_code, created_by, created_at, updated_at, is_active FROM recruitment_knowledge WHERE content_data LIKE '%科系%' OR content_data LIKE '%科%' ORDER BY created_at DESC LIMIT 1";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
        ...
        if (strpos($question_lower, '學費') !== false || strpos($question_lower, '費用') !== false) {
            $sql = "SELECT question, answer, source_type, source_label, department_code, created_by, created_at, updated_at, is_active FROM recruitment_knowledge WHERE content_data LIKE '%學費%' ORDER BY created_at DESC LIMIT 1";
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $result = $stmt->get_result();
...
function getRelevantTrainingData($question) {
    try {
        ...
        if (empty($where_conditions)) {
            // 如果沒有關鍵詞，返回最近的幾筆資料
            $sql = "SELECT question, answer, source_type, source_label, department_code, created_by, created_at, updated_at, is_active FROM recruitment_knowledge ORDER BY created_at DESC LIMIT 3";
            $stmt = $conn->prepare($sql);
        } else {
            // 使用通用相關性排序 - 根據關鍵詞匹配數量排序
            $relevance_order = buildRelevanceOrder($keywords);
            
            $sql = "SELECT question, answer, source_type, source_label, department_code, created_by, created_at, updated_at, is_active FROM recruitment_knowledge WHERE " . implode(' OR ', $where_conditions) . " ORDER BY " . $relevance_order . ", created_at DESC LIMIT 5";
            $stmt = $conn->prepare($sql);
...
function getDepartmentAnswer($question) {
    try {
        $conn = getOllamaDatabaseConnection();
        $sql = "SELECT answer FROM recruitment_knowledge WHERE question LIKE '%科系%' AND is_active=1 ORDER BY created_at DESC LIMIT 1";
...
function getCreatorAnswer($question) {
    try {
        ...
        // 查詢創造者相關資料
        $sql = "SELECT question, answer, source_type, source_label, department_code, created_by, created_at, updated_at, is_active FROM recruitment_knowledge WHERE content_data LIKE '%創造者%' OR content_data LIKE '%創作者%' ORDER BY created_at DESC LIMIT 1";
        $stmt = $conn->prepare($sql);
        $stmt->execute();
        $result = $stmt->get_result();
