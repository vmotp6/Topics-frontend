...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if (!empty($missing_columns)) {
            ...
            foreach ($missing_columns as $column_name => $definition) {
                echo "<li>$column_name ($definition)</li>\n";
            }
            echo "</ul>\n";
            
            echo "<h2>3. 添加缺少的欄位</h2>\n";
            foreach ($missing_columns as $column_name => $definition) {
                try {
                    $sql = "ALTER TABLE cooperation_applications ADD COLUMN $column_name $definition";
                    $pdo->exec($sql);
                    echo "✅ 成功添加欄位: $column_name<br>\n";
                } catch (PDOException $e) {
                    echo "❌ 添加欄位失敗 $column_name: " . $e->getMessage() . "<br>\n";
                }
            }
        } else {
            echo "✅ 所有必要欄位都已存在<br>\n";
    ...
    echo "<h3>✅ 資料庫結構修復成功！</h3>\n";
    echo "<p>現在您可以正常使用產學合作申請表了。</p>\n";
    echo "</div>\n";
