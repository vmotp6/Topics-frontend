-- =====================================================
-- 修復所有使用 username/email 而非 user_id 的表（安全版本）
-- 將這些表正規化為使用 user_id
-- 只在原始表存在時才進行遷移
-- =====================================================
...
-- 遷移數據（僅在原始表存在時）
-- MySQL 不支援在 INSERT ... SELECT 中使用 WHERE EXISTS 檢查表，改用條件插入
...
-- 注意：如果 user_activity 表不存在，此語句會失敗，但會在 PHP 中被忽略
...
-- 注意：如果 unread_notifications 表不存在，此語句會失敗，但會在 PHP 中被忽略
...
-- 注意：如果 notification_sent_log 表不存在，此語句會失敗，但會在 PHP 中被忽略
...
-- 注意：如果 message_likes 表不存在，此語句會失敗，但會在 PHP 中被忽略
...
SELECT '注意：如果原始表不存在，只會創建正規化表和視圖，不會進行數據遷移' AS note;
