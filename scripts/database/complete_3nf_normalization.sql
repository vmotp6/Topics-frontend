-- =====================================================
-- 完整資料庫第三正規化（3NF）腳本
-- 目標：將整個 topics_good 資料庫正規化至第三正規化（3NF）
-- 日期：2025
-- =====================================================
-- 
-- 此腳本會：
...
-- 執行順序：
-- 1. 先執行此腳本創建表結構
-- 2. 再執行 migrate_data_to_3nf.sql 遷移數據
-- 3. 最後執行 verify_3nf_compliance.sql 驗證結果
-- =====================================================
...
-- 如果表已存在但缺少 education_level 欄位，則添加
SET @col_exists = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() 
    AND table_name = 'grades' 
    AND column_name = 'education_level');
SET @sql = IF(@col_exists = 0, 
    'ALTER TABLE grades ADD COLUMN education_level ENUM(''國中'', ''高中'', ''專科'', ''大學'') DEFAULT ''專科'' COMMENT ''教育層級'' AFTER level',
    'SELECT ''Column education_level already exists''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 如果表已存在但缺少 level 欄位，則添加
SET @col_exists = (SELECT COUNT(*) FROM information_schema.columns 
    WHERE table_schema = DATABASE() 
    AND table_name = 'grades' 
    AND column_name = 'level');
SET @sql = IF(@col_exists = 0, 
    'ALTER TABLE grades ADD COLUMN level INT NOT NULL DEFAULT 1 COMMENT ''年級層級'' AFTER name',
...
CREATE TABLE IF NOT EXISTS enrollment_applications_normalized (
    ...
    email VARCHAR(255) NULL,
    junior_high_school_id INT NULL COMMENT '關聯到 schools 表（如果存在）',
    ...
    INDEX idx_recommended_teacher_user_id (recommended_teacher_user_id),
...
LEFT JOIN message_types mt ON p.message_type_id = mt.id;
...
LEFT JOIN message_types mt ON gm.message_type_id = mt.id;
...
SELECT '下一步：執行 migrate_data_to_3nf.sql 遷移數據' AS next_step;
